...
import cn.gorun8.easyfk.base.util.KeyStoreUtil;
...
/**
 * ComponentConfig - Component configuration class for easyfk-application.xml
 *
 */
public class ComponentConfig {

    public static final String module = ComponentConfig.class.getName();
    public static final String EASYFK_COMPONENT_XML_FILENAME = "easyfk-application.xml";
                ...
                synchronized (ComponentConfig.class) {
                    ...
                    if (componentConfig == null) {
                        ...
                        if (componentConfigs.containsKey(componentConfig.getGlobalName())) {
                            Debug.logWarning("WARNING: Loading easyfk-application using a global name that already exists, will over-write: " + componentConfig.getGlobalName(), module);
                        }
    ...
    /**
     * 获取可用的应用列表
     * @param enableAppNameList
     * @return
     */
    public static List<WebappInfo> getAllWebappResourceInfos(String enableAppNameList) {
        List<WebappInfo> webappInfos = FastList.newInstance();
        for (ComponentConfig cc: getAllComponents()) {
            if (enableAppNameList == null || enableAppNameList.contains(cc.getComponentName())) {
    ...
    protected ComponentConfig(String globalName, String rootLocation) throws ComponentException {
        ...
        String xmlFilename = rootLocation + "/" + EASYFK_COMPONENT_XML_FILENAME;
        URL xmlUrl = UtilURL.fromFilename(xmlFilename);
        if (xmlUrl == null) {
            throw new ComponentException("Could not find the " + EASYFK_COMPONENT_XML_FILENAME + " configuration file in the component root location: " + rootLocation);
        }

        Document easyfkComponentDocument = null;
        try {
            easyfkComponentDocument = UtilXml.readXmlDocument(xmlUrl, true);
        ...
        Element easyfkComponentElement = easyfkComponentDocument.getDocumentElement();
        this.componentName = easyfkComponentElement.getAttribute("name");
        this.enabled = "true".equalsIgnoreCase(easyfkComponentElement.getAttribute("enabled"));
        if (UtilValidate.isEmpty(this.globalName)) {
            this.globalName = this.componentName;
        }

        // resource-loader - resourceLoaderInfos
        for (Element curElement: UtilXml.childElementList(easyfkComponentElement, "resource-loader")) {
        ...
        for (Element curElement: UtilXml.childElementList(easyfkComponentElement, "entity-resource")) {
        ...
        for (Element curElement: UtilXml.childElementList(easyfkComponentElement, "service-resource")) {
        ...
        for (Element curElement: UtilXml.childElementList(easyfkComponentElement, "test-suite")) {
        ...
        for (Element curElement: UtilXml.childElementList(easyfkComponentElement, "keystore")) {
            KeystoreInfo keystoreInfo = new KeystoreInfo(this, curElement);
            this.keystoreInfos.add(keystoreInfo);
        }

        // webapp - webappInfos
        for (Element curElement: UtilXml.childElementList(easyfkComponentElement, "webapp")) {
